...
type Tab='network'|'builder'|'supply'|'sewer'|'storm'|'general'
...
export default function HydraulicsPage(){
 ...
 <nav className="hyd-tabs">{([['network','Projeto'],['builder','Aparelhos'],['supply','Abastecimento'],['sewer','Esgotos'],['storm','Pluviais'],['general','Geral']] as const).map(([k,l])=><button className={tab===k?'active':''} onClick={()=>setTab(k)} key={k}>{l}</button>)}</nav>
 {tab==='network'&&<NetworkDesigner/>}{tab==='builder'&&<SanitaryBuilder/>}{tab==='supply'&&<Supply/>}{tab==='sewer'&&<Sewer/>}{tab==='storm'&&<Storm/>}{tab==='general'&&<General/>}
 </div>
}
