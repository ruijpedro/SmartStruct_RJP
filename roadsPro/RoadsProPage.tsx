...
export default function RoadsProPage(){
 ...
 {tab==='xyplan'&&<>
  ...
  <section className="panel"><h3>Planta XY · ligação automática XY ↔ PK</h3>
   <XYPlanSvg alignment={alignment} elems={linkedElems} scale={xyScale} onPick={(x,y)=>{const p=projectXYToAlignment(alignment,x,y);const s:'E'|'D'|'T'=p.offset<-.2?'E':p.offset>.2?'D':'T';setXyElems([...xyElems,{id:`D${xyElems.length+1}`,kind:selectedKind,x,y,pk:p.pk,offset:Math.abs(p.offset),side:s}])}}/>
  </section>
...
function XYPlanSvg({alignment,elems,scale,onPick}:{alignment:AlignmentVertex[];elems:XYDrainageElement[];scale:number;onPick:(x:number,y:number)=>void}){
 ...
 const click=(ev:React.MouseEvent<SVGSVGElement>)=>{const r=ev.currentTarget.getBoundingClientRect();const vx=(ev.clientX-r.left)/r.width*W;const vy=(ev.clientY-r.top)/r.height*H;onPick(cx+(vx-W/2)/k,cy-(vy-H/2)/k)}
